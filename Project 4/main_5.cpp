...
int main (int argc, char* argv[]){ 
	...
	for (int n_cycles = 1; n_cycles <= max_cycles; n_cycles++){	// loop over the different numbers of cycles
		...
		avg = average(0)/double(L*L);		
		myfile<<n_cycles<<" "<<setw(15) << setprecision(8) <<avg/(double)(n_cycles)<<" "<<average(2)*(1./(n_cycles*L*L))<<endl;
	}
	
	myfile.close();
	
	average(0) = average(0)*(1./(double)(max_cycles*L*L));
