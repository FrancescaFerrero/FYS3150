...
int main (){ 
 ...
 //double n_step = 1.0e5; // number of steps
...
// Add the particle to both of the traps
...
// no interactions
	...
	/*
	// Open file to save position and velocity values - no interaction
	ofstream myfile1a_nointer;
	myfile1a_nointer.open ("r1_nointer_RK_"+std::to_string((int)n_step)+".txt");
	myfile1a_nointer<<R.slice(0).t();
	myfile1a_nointer.close();
	
	ofstream myfile1b_nointer;
	myfile1b_nointer.open ("r1_nointer_FE_"+std::to_string((int)n_step)+".txt");
	myfile1b_nointer<<R_eu.slice(0).t();
	myfile1b_nointer.close();
	*/
...
for (int i=0; i<n_step; i++){
	...
	ofstream myfile5;
	myfile5.open ("r_analytic."+std::to_string((int)n_step)+".txt");
	myfile5<<R_an.t();
	myfile5.close();
	
	

// Calculte absoloute error - RK and FE
